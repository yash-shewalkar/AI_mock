...
export default function InterviewForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const role =
        selectedRole === "other" ? formData.get("other_role") : selectedRole;
      ...
      router.push(
        `/interview/results?data=${encodeURIComponent(JSON.stringify(result))}`
      );
    } catch (error) {
  ...
  return (
    <div>
       <h2 className="mx-auto text-blue-400 text-center" >Tell us what are you preparing for?</h2>
      ...
      >
        <div>
          ...
          <select
            name="type"
            className="w-full p-2.5 border border-gray-300 rounded-lg bg-slate-900 focus:ring-blue-500 focus:border-blue-500 text-white"
            required
            defaultValue="mixed"
          >
        ...
        >
          {isSubmitting ? "Preparing..." : "Prepare Interview"}
        </button>
      </form>
       </section>
    </div>
